...
ten is marked $\dagger$ and is not treated as a finding. We adopted this check
after observing that one borderline cell changed verdict when the resample
count changed, and we regard reporting it as preferable to selecting the
setting that produced the more favourable answer.

All comparisons are paired on the folds both conditions completed. Where the
two conditions completed different numbers of folds, the paired count is the
one reported, and any comparison we cannot make paired we mark as such rather
than differencing two unequally estimated means.
...
anything, slightly negative. This is the representation the abstraction was
designed to provide, and on this evidence it provides nothing.

\textbf{Combining them does not recover the grammar kind's advantage.} The sum
reaches AUPRC $0.164$ against $0.215$ for $\kappa$ alone. We report this
comparison with a caveat we consider important: the two conditions completed
different numbers of folds ($9$ against $5$), so the gap is not a paired
contrast of the kind the rest of the table reports, and we do not test it.
What the table does establish is the weaker and safer statement that adding
the taxonomy to the grammar kind does not improve on the grammar kind ---
which is consistent with Section~\ref{sec:rq_why} --- not that it actively
harms it.

One cell deserves explicit caution. The AUPRC gain of the combined
representation, $+0.022$, has a lower bound of $+0.0002$: it excludes zero by
two ten-thousandths, and moving from $1000$ to $2000$ bootstrap resamples
moves that bound across zero. We report the interval as computed at our
pre-declared setting and rest no claim on it. Cells whose verdict varies
across bootstrap seeds are marked $\dagger$ throughout.

Absolute performance remains modest and we state it plainly: AUPRC $0.215$
against a base rate of $0.088$ is a lift of about $2.4\times$, and
...
\input{tab_rq4_granularity}

We compare granularities against \emph{each other} rather than each against its
own lexical baseline. The three runs share a fold assignment --- fold $i$ holds
out the same repositories in every run, which we verify programmatically rather
than assume --- so a paired comparison of $|\mathcal{T}| = 16$ against
$|\mathcal{T}| = 8$ varies granularity and nothing else. Comparing each
granularity to its own baseline would not: those baselines are themselves
estimated on whichever folds each run completed, and differencing two such
estimates confounds granularity with fold composition. An earlier version of
this analysis did exactly that and read a $+0.010$ AUC ``refinement benefit''
off the result; the paired comparison shows that quantity to be an artifact.
...
granularity does not matter, and the $8 \rightarrow 16$ step is the one the
taxonomy design actually turns on, since it separates allocation from release
from block copy. The $|\mathcal{T}| = 32$ run completed a single fold; we
report its interval for completeness but draw no conclusion from a comparison
...
\begin{itemize}
    ...
    \item \textbf{Granularity does not matter.} Refining the alphabet from
          eight to sixteen types changes AUC by $-0.002$ $[-0.023, +0.026]$
...
scheme weighted by prototype similarity. Both were removed rather than
reported: the pipeline that produced their numbers contained defects identified
during the analysis validation of Section~\ref{sec:rq_analysis}, and we
considered it unsound to carry forward results we could not reproduce under the
corrected implementation. Neither component appears in the system evaluated
here, and no claim in this paper depends on either. We describe them at all
only because omitting a component that appeared in a prior version, without
saying why, is the kind of silent deletion a reader is entitled to know about.
