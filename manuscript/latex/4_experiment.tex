...
Table~\ref{tab:rq2b_information} and Figure~\ref{fig:redundancy} give a
complete explanation. The taxonomy is
...
\begin{table}[t]
\centering
\caption{RQ4: taxonomy granularity (BigVul, GGNN). Each row compares the
operation$+$kind representation at that granularity against the lexical
baseline computed on the same folds.}
\label{tab:rq4_granularity}
\begin{tabular}{lrrr}
\toprule
Granularity & Lexical AUC & Operation$+$kind AUC & $\Delta$ \\
\midrule
$|\mathcal{T}| = 8$  & 0.587 & 0.668 & $+0.082$ \\
$|\mathcal{T}| = 16$ & 0.583 & 0.675 & $+0.092$ \\
$|\mathcal{T}| = 32$ & \multicolumn{3}{c}{not run to completion --- see note} \\
\bottomrule
\end{tabular}
\end{table}

Refining the alphabet from eight to sixteen types changes the effect by
$+0.010$ AUC, well inside the fold-to-fold variation of either configuration
($\sigma \approx 0.03$--$0.05$) and inside the confidence intervals of both. On
this evidence, granularity does not matter. We report the $|\mathcal{T}| = 32$
configuration as not run to completion rather than quoting a partial result: a
single fold yields no interval, and the comparison we would draw from it is not
one we would defend. The claim we make is therefore restricted to the
$8 \rightarrow 16$ refinement, which is the step the taxonomy design actually
turns on (it separates allocation from release from block copy).

The class distribution explains why, and it was visible before the experiment
was run (Table~\ref{tab:node_dist}): the memory-operation class that motivates
the refinement accounts for $0.43\%$ of AST nodes. Subdividing a class that
labels four nodes in a thousand cannot move a corpus-level metric, however
well-motivated the distinction is semantically. Taken with
Section~\ref{sec:rq_why}, this completes a consistent account: the taxonomy is
almost entirely redundant with the grammar kind, and the part that is not
redundant is too rare to matter.
...
\begin{itemize}
    ...
    \item \textbf{As a representation} it is not what matters. Abandoning
          ...
          all eight corpus $\times$ backbone comparisons, by roughly three times
          the spread between the architectures themselves; but the operation
          taxonomy alone is indistinguishable from lexical features
          ($p = 0.89$), and the grammar kind carries the effect.
    \item \textbf{The null result has a mechanism.} $\phi$ is $98.2\%$
          predictable from the grammar kind, so it supplies almost nothing the
          parser has not already provided.
    \item \textbf{Granularity is irrelevant} for the same reason, compounded by
          the rarity of the classes a finer taxonomy separates.
\end{itemize}

We note explicitly what we do \emph{not} report. An earlier version of this
work included a learned edge-weighting module and a federated aggregation
scheme weighted by prototype similarity. Both were removed rather than reported:
the pipeline that produced their numbers contained defects identified during the
analysis validation of Section~\ref{sec:rq_analysis}, and we considered it
unsound to carry forward results we could not reproduce under the corrected
implementation. Neither component appears in the system evaluated here.
