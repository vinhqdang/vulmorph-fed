...
\begin{proposition}[Per-round privacy of the prototype release, conditional on the encoder]
\label{prop:dp}
Fix a client $k$ and \emph{condition on its encoder} $f_{\theta_k}$. Let two neighbouring datasets differ in one function. Releasing the noisy bank $\{\hat{\mathbf{p}}_c^{(k)}\}$ of Eq.~\eqref{eq:laplace} satisfies $2\varepsilon$-differential privacy with respect to that record.
\end{proposition}

\begin{proof}
Each per-sample embedding is projected onto the $\ell_1$ ball of radius $R$ (Eq.~\ref{eq:prototype}), so with $f_{\theta_k}$ held fixed, replacing one function changes the affected class mean by at most $\Delta_1 = 2R / N_{c,k}$ in $\ell_1$ norm; the Laplace mechanism with scale $\Delta_1/\varepsilon$ is $\varepsilon$-DP for that row~\cite{dwork2014algorithmic}. Under the replacement neighbouring relation a record change can affect \emph{two} rows --- it removes a record from the slot of the old label and adds one to the slot of the new label --- so parallel composition does not apply and the two affected rows compose sequentially, giving $2\varepsilon$.
\end{proof}

\begin{corollary}[End-to-end budget]
Over $T$ rounds, sequential composition yields $\varepsilon_{\mathrm{tot}} = 2T\varepsilon$. All privacy tables report both the per-round and the composed budget (e.g.\ $\varepsilon = 2$ per round over $T = 10$ rounds gives $\varepsilon_{\mathrm{tot}} = 40$).
\end{corollary}

\paragraph{What this guarantee does and does not cover.}
We state the scope precisely, because it is easy to overclaim here. The conditioning in Proposition~\ref{prop:dp} is load-bearing: $\theta_k$ is trained on the client's raw data \emph{without} a privacy mechanism, so the map from dataset to embedding function is not itself differentially private. Changing one training record perturbs $\theta_k$, and hence every embedding, by an amount we do not bound. Consequently the guarantee is a statement about the \emph{release mechanism applied to the prototypes given a fixed encoder}, not an end-to-end DP guarantee over the training pipeline, and the deployed detector of Eq.~\eqref{eq:ensemble} --- which contains the raw-trained encoders --- carries no DP guarantee at all. Two further leakage channels remain outside the analysis: the noise scale $2R/(N_{c,k}\varepsilon)$ depends on the private count $N_{c,k}$, and the \emph{support pattern} of a client's occupied slots is transmitted unnoised, revealing which weakness classes a client possesses. Closing these gaps requires DP-SGD during local training (with its budget composed into $\varepsilon_{\mathrm{tot}}$), a count-independent noise scale, and padding or noising of the support pattern; we regard this as the principal item of future work rather than a solved problem, and we accordingly avoid the word ``private'' unqualified when describing the deployed system.
