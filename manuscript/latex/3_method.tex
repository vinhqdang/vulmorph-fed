...
Full Code Property Graph (CPG) extraction requires a heavyweight static-analysis engine such as Joern~\cite{yamaguchi2014modeling}, which is impractical to deploy at every federated edge client and complicates exact reproduction. Our implementation therefore operates on a \emph{lightweight lexical dependence graph} that approximates the CPG views most relevant to our abstraction. For a function $f$ with token sequence $t_1, \dots, t_n$ (we retain the first $n \le 100$ tokens produced by a C/C++ tokenizer that merges multi-character operators such as \texttt{->}, \texttt{==}, \texttt{<<=}), the graph $G = (\mathcal{V}, \mathcal{E})$ has:
\begin{itemize}
    \item one node per token, $|\mathcal{V}| = n$;
    \item \emph{natural-code-sequence} (NCS) edges $\mathcal{E}_{NCS} = \{(t_i, t_{i+1})\}$, as used in Devign~\cite{zhou2019devign};
    \item \emph{data-dependence proxy} edges $\mathcal{E}_{DD}$ linking successive occurrences of the same identifier (a def-use approximation), excluding language keywords.
\end{itemize}
The edge set is $\mathcal{E} = \mathcal{E}_{NCS} \cup \mathcal{E}_{DD}$; on our corpora this yields on average 70--90 nodes and 77--98 edges per function. We state explicitly that these are \emph{not} compiler-grade CPGs; Section~\ref{sec:discussion} discusses this trade-off, and the released implementation isolates graph construction behind a single interface so that Joern-derived CPGs can be substituted without touching the rest of the framework.
...
We define a fixed semantic taxonomy $\mathcal{T}$ of $|\mathcal{T}| = 8$ operation types, listed exhaustively in Table~\ref{tab:taxonomy} together with the reserved fallback type \texttt{UNKNOWN}. A node therefore takes one of $|\mathcal{T}| + 1 = 9$ categories, and node features are embeddings (equivalently, one-hot encodings) of dimension $|\mathcal{T}| + 1$ --- the \texttt{UNKNOWN} type has its own slot. For the taxonomy-size sensitivity analysis (RQ2b) we additionally define strictly nested refinements with $|\mathcal{T}| = 16$ and $|\mathcal{T}| = 32$ (e.g., \texttt{MEMORY\_ACCESS} splits into \texttt{MEMORY\_ALLOC}, \texttt{MEMORY\_FREE}, \texttt{MEMORY\_COPY}, \texttt{STRING\_OP}); the complete lists ship with the source code.
...
The mapping $\phi_{\mathrm{type}}: \mathcal{V} \rightarrow \mathcal{T} \cup \{\texttt{UNKNOWN}\}$ is a deterministic, context-aware rule set evaluated over a token and its immediate neighbours, in priority order:
\begin{enumerate}
    \item an identifier followed by `\texttt{(}' maps to the memory/string API class if it appears on a fixed allow-list (Table~\ref{tab:taxonomy}, row 1; the full allow-lists are in the released code), to the control class if it is a control keyword, and to \texttt{CALL\_SITE} otherwise --- \emph{user-defined functions, macros and unresolved calls all map to \texttt{CALL\_SITE}}; no name resolution is required;
    \item an identifier followed by `\texttt{[}' maps to \texttt{ARRAY\_INDEX};
    \item control keywords map to \texttt{CONTROL\_BRANCH};
    \item \texttt{->}, and \texttt{*}/\texttt{\&} in prefix (unary) position, map to \texttt{PTR\_DEREF}; \texttt{*}/\texttt{\&} in infix position map to \texttt{ARITH\_OP};
    \item operator tokens map to \texttt{ARITH\_OP}, \texttt{COMPARISON} or \texttt{ASSIGN} as listed in Table~\ref{tab:taxonomy};
    \item every remaining token maps to \texttt{UNKNOWN}.
\end{enumerate}
The rules are implemented once at the finest ($|\mathcal{T}|=32$) granularity and projected onto the coarser taxonomies, so all three taxonomy sizes share a single rule set. $\phi_{\mathrm{type}}$ is a discrete label assignment and is \emph{not} differentiable; the only learned part of VCSA is the edge-weighting module below.

On our four evaluation corpora, 25--26\% of nodes receive a non-\texttt{UNKNOWN} type. The abstraction does not delete nodes: all $n$ nodes are retained with their graph structure, but their features are collapsed from a $\sim$10,000-token vocabulary onto the $|\mathcal{T}|+1$ abstract categories. This many-to-one collapse --- roughly three orders of magnitude --- is what removes project-specific lexical information from everything the model computes and, downstream, from everything a client transmits (Section~\ref{sec:privacy}).
...
\begin{itemize}
    ...
    \item \textbf{CWE labels condition only the prototypes.} A vulnerable function annotated with several CWEs contributes to the prototype of its \emph{first-listed (primary)} CWE. Benign functions receive no prototype.
    \item \textbf{CWE vocabulary.} The prototype bank has a fixed size $|\mathcal{C}| = 10$: the nine most frequent CWE types in the training corpus receive dedicated slots and all remaining types share an \texttt{OTHER} slot. This bucketing is computed once per dataset and reported with the released configurations. Datasets without per-function CWE annotations (Devign) degenerate gracefully to a single vulnerable-prototype slot.
\end{itemize}

\subsection{Morphology-Conditioned Federated Prototype Aggregation (MCFPA)}
\label{sec:mcfpa}

FedAvg-style parameter averaging~\cite{mcmahan2017fedavg} assumes near-IID clients; vulnerability data is extremely non-IID in its CWE composition. MCFPA instead federates \emph{CWE-conditioned prototypes}. After local training, client $k$ computes, for each CWE bucket $c$ with local support $N_{c,k} > 0$,
\begin{equation}
...
\label{eq:prototype}
\end{equation}
i.e., the centroid of the $\ell_1$-clipped embeddings of its local vulnerable functions of type $c$. Buckets with $N_{c,k} = 0$ are transmitted as zero rows and carry no information.
...
\begin{proof}
Each per-sample embedding is projected onto the $\ell_1$ ball of radius $R$ (Eq.~\ref{eq:prototype}), so replacing one function changes the class mean by at most $\Delta_1 = 2R / N_{c,k}$ in $\ell_1$ norm. The Laplace mechanism with scale $\Delta_1 / \varepsilon$ is therefore $\varepsilon$-DP for that row \cite{dwork2014algorithmic}. A function carries exactly one primary CWE (Section~\ref{sec:label_model}), so a record change affects exactly one row of the bank; by parallel composition across the disjoint buckets the entire release is $\varepsilon$-DP.
\end{proof}
